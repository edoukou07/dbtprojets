  ...
  create  table "sigeti_node_db"."dwh_dimensions"."dim_impenses__dbt_tmp"
...
with impenses_source as (
    select * from "sigeti_node_db"."dwh_staging"."stg_impenses"
),
...
entreprises as (
    select 
        ...
        secteur_activite
    from "sigeti_node_db"."dwh_staging"."stg_entreprises"
),

final as (
    select
        -- Clé surrogate
        md5(cast(coalesce(cast(i.impense_id as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as impense_key,
