...
UPDATE dwh_facts.fait_collectes
SET duree_reelle_jours = EXTRACT(DAY FROM (date_cloture::date - date_debut::date))
WHERE date_debut IS NOT NULL 
  AND date_cloture IS NOT NULL
  AND duree_reelle_jours <= 0;
SELECT 
...
collectes_aggregees AS (
    SELECT
        ...
        ROUND(AVG(CASE WHEN c.duree_reelle_jours > 0 THEN c.duree_reelle_jours ELSE 0 END)::numeric, 1) as duree_moyenne_collecte
    FROM collectes c
