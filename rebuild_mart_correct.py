...
print("=== Clear and rebuild mart (FINAL FIX) ===")
...
occupation_data as (
    SELECT
        ...
        SUM(CASE WHEN l.statut::text = 'disponible' THEN 1 ELSE 0 END) as lots_disponibles,
        SUM(CASE 
            WHEN EXISTS (
        ...
        SUM(CASE WHEN l.viabilite = true THEN 1 ELSE 0 END) as lots_viabilises,
        ROUND(
            (SUM(CASE WHEN l.viabilite = true THEN 1 ELSE 0 END)::numeric 
             / NULLIF(COUNT(*), 0)::numeric * 100), 
            2
...
connection.commit()
print('✓ Mart rebuilt with fixed logic!')

print("\n=== FINAL VERIFICATION: All zones ===")
cursor.execute('''
SELECT 
    nom_zone,
    nombre_total_lots,
...
print("Zone | Total | Attrib | Dispo | Réservé | Check | Status | Taux")
print("-" * 70)
for nom, total, attrib, dispo, reserves, check, status, taux in cursor.fetchall():
    print(f'{nom:30s} | {total:5d} | {attrib:6d} | {dispo:5d} | {reserves:7d} | {check:5d} | {status:4s} | {taux:6.2f}%')

print("\n✓ All zones correctly account for all lots!")
