{{
    config(
        ...
        schema='facts',
        unique_key='workflow_impense_key',
        on_schema_change='append_new_columns',
        tags=['impenses', 'workflow', 'P2']
    )
}}
...
with workflow as (
    select
        ...
        date_action,
        created_at
    from {{ ref('stg_workflow_historiques') }}
    {% if is_incremental() %}
    where date_action > (select coalesce(max(date_action), '1900-01-01') from {{ this }})
    {% endif %}
),
...
final as (
    select
        ...
        {{ dbt_utils.generate_surrogate_key(['h.workflow_historique_id', 'h.date_action']) }} as workflow_impense_key,
    ...
    left join {{ ref('dim_impenses') }} di on h.impense_id = di.impense_id
    left join {{ ref('dim_phases_impenses') }} dp on h.action = dp.action_code
    left join {{ ref('dim_agents') }} a on h.user_id = a.agent_id
)
