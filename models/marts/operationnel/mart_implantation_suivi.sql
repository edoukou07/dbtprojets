...
suivis as (
    ...
    from {{ source('sigeti_source', 'suivis_implantation') }}
),
...
demandes as (
    select 
        da.id as demande_id,
        dal.lot_id
    from {{ source('sigeti_source', 'demandes_attribution') }} da
    left join {{ source('sigeti_source', 'demande_attribution_lots') }} dal 
...
lots as (
    select 
        id as lot_id,
        zone_id
    from {{ source('sigeti_source', 'lots') }}
),
