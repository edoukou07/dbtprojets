{{
    config(
        ...
        tags=['impenses', 'performance', 'P2'],
        indexes=[
            {'columns': ['phase_id']},
            {'columns': ['action']},
            {'columns': ['niveau_goulot']},
            {'columns': ['est_goulot_etranglement']}
        ]
    )
}}
...
with stats_par_phase as (
    ...
    inner join {{ ref('dim_phases_impenses') }} p on f.phase_impense_key = p.phase_impense_key
