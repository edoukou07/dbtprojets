{{
    config(
        ...
        tags=['impenses', 'suivi', 'P2'],
        indexes=[
            {'columns': ['impense_id']},
            {'columns': ['statut_actuel']},
            {'columns': ['phase_actuelle']},
            {'columns': ['est_en_retard']},
            {'columns': ['zone_industrielle_code']},
...
with metriques_dossier as (
    ...
    from {{ ref('fait_workflow_impenses') }} f
    left join {{ ref('dim_phases_impenses') }} p on f.phase_impense_key = p.phase_impense_key
...
agent_principal as (
    ...
    from {{ ref('fait_workflow_impenses') }}
...
lots_info as (
    select
        ...
        z.zone_libelle,
        z.zone_adresse
    from {{ source('sigeti_source', 'lots') }} l
